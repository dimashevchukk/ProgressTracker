...
from tracker.models import MediaItem, MediaType, Note, Profile, UserMedia
...
class UserRegistrationViewTest(TestCase):
    ...
    def test_registration_view_post_invalid_data(self):
        data = {"username": "testuser", "password1": "pass", "password2": "different"}
...
class UserLibraryViewTest(TestCase):
    def setUp(self):
        ...
        self.media1 = MediaItem.objects.create(
            title="Test Movie 1", type=MediaType.MOVIE
        )
        self.media2 = MediaItem.objects.create(title="Test Book 1", type=MediaType.BOOK)
...
class MediaDetailViewTest(TestCase):
    def setUp(self):
        ...
        self.note1 = Note.objects.create(
            user_media=self.user_media, title="First title", text="First note"
        )
        self.note2 = Note.objects.create(
            user_media=self.user_media, title="Second title", text="Second note"
        )

    def test_media_detail_anonymous_user(self):
        url = reverse(
            "tracker:media-detail",
            kwargs={"type": self.media.type, "pk": self.media.pk},
        )
...
class AddMediaToLibraryTest(TestCase):
    def setUp(self):
        ...
        self.media = MediaItem.objects.create(title="Test Media", type=MediaType.MOVIE)
        self.url = reverse(
            "tracker:media-add", kwargs={"type": self.media.type, "pk": self.media.pk}
        )
